...
// replace current CSV_CANDIDATES line with this
...
const initPyCode = `
import io, pandas as pd, re, sys
from collections import Counter

# Defensive checks on injected csv_text
if 'csv_text' not in globals():
    raise ValueError("Injected variable 'csv_text' not present in Python globals.")

if csv_text is None:
    raise ValueError("Injected csv_text is None. The JS fetch likely failed or returned no body.")

if not isinstance(csv_text, str):
    raise ValueError("Injected csv_text is not a string (type: {}).".format(type(csv_text)))

if csv_text.strip() == "":
    # show a short preview so user can see whether they got an HTML 404 page, etc.
    preview = (csv_text or "")[:1000]
    raise ValueError("CSV content appears empty. Preview (first 1000 chars):\\n" + preview)

# Try to read with pandas; catch common parsing errors and surface a helpful message.
try:
    df = pd.read_csv(io.StringIO(csv_text), encoding='utf-8')
except Exception as e:
    # include a short preview so we can see if it's an HTML 404 or garbled text
    preview = (csv_text or "")[:1000]
    raise ValueError("pd.read_csv failed: {}\\nPreview (first 1000 chars):\\n{}".format(e, preview))

if df is None:
    raise ValueError("pd.read_csv returned None (unexpected).")

# Ensure expected columns exist and report precisely which are missing
expected_cols = ['chapter', 'lemmas_str', 'word_count']
found_cols = [str(c) for c in df.columns.tolist()]
missing = [c for c in expected_cols if c not in found_cols]
if missing:
    raise ValueError("chap_lemmas.csv is missing required columns: {}. Found columns: {}. (Header row preview: {})"
                     .format(", ".join(missing), ", ".join(found_cols), ",".join(found_cols[:10])))
